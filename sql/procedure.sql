...
DELETE
FROM supplierWater
WHERE supplierWater.supplierID = supplierID
  and supplierWater.waterID = waterID;
END$$
...
CREATE PROCEDURE `selectMonthlyDelivery`(_ year CHAR (4), _ month CHAR (2))
BEGIN
  DECLARE
_yearmonth CHAR(6);
  SET
_yearmonth = CONCAT(_year,_month);

  IF
_yearmonth = '000000' THEN
SELECT *
FROM monthlyDelivery
...
BEGIN
SELECT *
FROM monthlyPurchase
ORDER BY yearmonth DESC;
END$$

CREATE PROCEDURE `selectMonthlySales`(_ year CHAR (4), _ month CHAR (2))
BEGIN
  DECLARE
_yearmonth CHAR(6);
  SET
...
BEGIN
SELECT orderDetail.ID ID,
       customID,
       waterID,
       number,
       custom.`name`  customName,
       water.`name`   waterName, time,
...
BEGIN
SELECT ID,
       `name`, size, cost, price, number quantity, state
FROM water, stock
WHERE state = 1
...
CREATE PROCEDURE `selectOrderDetailById`(orderDetailID INT)
BEGIN
SELECT *
FROM orderDetail
...
CREATE PROCEDURE `updateWater`(waterID INT, number INT)
BEGIN
UPDATE stock
SET number = number
